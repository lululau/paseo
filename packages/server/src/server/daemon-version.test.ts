...
describe("resolveDaemonVersion", () => {
  it("resolves server version by walking up to @lululau/paseo-server package.json", () => {
  ...
  it("throws when @lululau/paseo-server package metadata cannot be resolved", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
  ...
  it("throws when @lululau/paseo-server version is missing", () => {
    const root = createTempDir();
    writeFileSync(
      path.join(root, "package.json"),
